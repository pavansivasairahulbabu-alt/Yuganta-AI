...
export default function SecurePDFViewer({ document, courseId, token, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 sm:p-8">
            {/* Modal Container */}
            <div className="bg-[var(--bg-primary)] border border-gray-500/20 rounded-2xl shadow-2xl w-full max-w-5xl h-full max-h-[90vh] flex flex-col overflow-hidden flex-1 relative">

                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-500/20 bg-[var(--bg-card)]">
                    <h3 className="font-bold text-[var(--text-primary)] truncate pr-4">{document.name}</h3>
                    <button
                        onClick={onClose}
                ...
                {/* PDF Canvas Area */}
                <div
                    className="flex-1 overflow-y-auto custom-scrollbar bg-[#0a0a0a] flex justify-center py-8 relative select-none"
                    ...
                    {error ? (
                        ...
                        >
                            <Page
                                ...
                                width={Math.min(window.innerWidth * 0.9, 800)} // Responsive scaling
                            />
                        </Document>
                    )}
                </div>
                ...
                {numPages && (
                    <div className="flex items-center justify-between px-6 py-4 border-t border-gray-500/20 bg-[var(--bg-card)]">
                        <button
                            disabled={pageNumber <= 1}
                            onClick={() => setPageNumber(prev => prev - 1)}
                            className="flex items-center space-x-2 px-4 py-2 rounded-lg bg-gray-500/10 text-[var(--text-primary)] disabled:opacity-30 hover:bg-gray-500/20 transition-colors"
                        >
